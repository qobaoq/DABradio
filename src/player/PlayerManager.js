...
class PlayerManager {
    constructor(client, distubeOptions) {
        this.client = client;
        const BlocklistManager = require('../utils/BlocklistManager');
        ...
        const SongLogger = require('../utils/SongLogger');
        this.songLogger = new SongLogger();
  ...
  async handlePlaySong(queue, song) {
    try {
      ...
      // Log the song to Discord channel
      await this.songLogger.logSong(this.client, queue.voiceChannel, song, queue.songs[0].member.user);
      ...
      if (queue.textChannel) {
        ...
        const embed = new EmbedBuilder()
          ...
          .addFields(
            { name: 'Tip', value: `**Have fun**`, inline: true },
          )
          .setTimestamp();
